...
/******************************************************************************/
#ifdef USE_KOKKOS
void KernelAB_SOA_Base::timestepForce(myViewPDF dstrb_src, myViewPDF dstrb_tgt, int startIdx, int countIdx)
{
   ...
   Kokkos::parallel_for(myPolicy(startIdx, startIdx+countIdx),KOKKOS_CLASS_LAMBDA (const int fluidIdx)
   {
	Pdf f[3] = {_GRAVITY_, 0.0, 0.0};
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         rho += dstrb_src(nFluid * stencilIdx + fluidIdx);
         v[0] += dstrb_src(nFluid * stencilIdx + fluidIdx) * stencil_d_(_DIMS_*stencilIdx+0);
         v[1] += dstrb_src(nFluid * stencilIdx + fluidIdx) * stencil_d_(_DIMS_*stencilIdx+1);
         v[2] += dstrb_src(nFluid * stencilIdx + fluidIdx) * stencil_d_(_DIMS_*stencilIdx+2);
      }
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         ...
         int streamLoc = this_adjacency_d_(nFluid * stencilIdx + fluidIdx);
         dstrb_tgt(streamLoc) =
            (1.0-_OMEGA_) * dstrb_src(nFluid * stencilIdx + fluidIdx) +
            _OMEGA_ * eq +
...
q.parallel_for(sycl::range<1>{size_t(countIdx)},sycl::id<1>{size_t(startIdx)},[=,adjacency_d_=this->geometry_.adjacency_d_,stencil_d_=this->stencil_d_,weight_d_=this->weight_d_](sycl::id<1> fluidIdx)
...
#endif
   {
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         ...
         v[2] += dstrb_src[nFluid * stencilIdx + fluidIdx] * stencil_d_[_DIMS_*stencilIdx+2];
#else        
         v[0] += dstrb_src[nFluid * stencilIdx + fluidIdx] * stencil_[_DIMS_*stencilIdx+0];
         v[1] += dstrb_src[nFluid * stencilIdx + fluidIdx] * stencil_[_DIMS_*stencilIdx+1];
         v[2] += dstrb_src[nFluid * stencilIdx + fluidIdx] * stencil_[_DIMS_*stencilIdx+2];
#endif
      }
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
		...
		Pdf cdotv = _INVCS2_ * (stencil_d_[_DIMS_*stencilIdx+0] * v[0] +
         ...
         Pdf eq = weight_d_[stencilIdx] * rho * (1.0 + cdotv + 0.5 * cdotv * cdotv - 0.5 * vdotv);
         Pdf fdist = weight_d_[stencilIdx] * (cdotf - fdotv + cdotv * cdotf);
         int streamLoc = adjacency_d_[nFluid * stencilIdx + fluidIdx];
#else
         ...
         int streamLoc = geometry_.adjacency_[nFluid * stencilIdx + fluidIdx];
#endif
         dstrb_tgt[streamLoc] =
            (1.0-_OMEGA_) * dstrb_src[nFluid * stencilIdx + fluidIdx] +
            _OMEGA_ * eq +
