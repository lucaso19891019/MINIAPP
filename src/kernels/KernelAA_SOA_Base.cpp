...
/******************************************************************************/
#ifdef USE_KOKKOS
void KernelAA_SOA_Base::timestepEvenForce(myViewPDF dstrb, int startIdx, int countIdx)
{
   ...
   Kokkos::parallel_for(myPolicy(startIdx, startIdx+countIdx),KOKKOS_CLASS_LAMBDA (const int fluidIdx)
   {
	Pdf f[3] = {_GRAVITY_, 0.0, 0.0};
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         tempDstrb[stencilIdx] = dstrb(nFluid * opp_d_(stencilIdx) + fluidIdx);
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         ...
         dstrb(nFluid * stencilIdx + fluidIdx) = (1.0-_OMEGA_) * tempDstrb[stencilIdx] + _OMEGA_ * eq+
...
/******************************************************************************/
void KernelAA_SOA_Base::timestepOddForce(myViewPDF dstrb, int startIdx, int countIdx)
{
   ...
   Kokkos::parallel_for(myPolicy(startIdx, startIdx+countIdx),KOKKOS_CLASS_LAMBDA (const int fluidIdx)
   {
 	Pdf f[3] = {_GRAVITY_, 0.0, 0.0};
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         tempLoc[stencilIdx] = this_adjacency_d_(nFluid * stencilIdx + fluidIdx);
      ...
      for (int stencilIdx = 0; stencilIdx < _LATTICESIZE_; stencilIdx++)
      {
         ...
         Pdf eq = weight_d_(stencilIdx) * rho * (1.0 + cdotv + 0.5 * cdotv * cdotv - 0.5 * vdotv);
         Pdf fdist = weight_d_(stencilIdx) * (cdotf - fdotv + cdotv * cdotf);
         dstrb(tempLoc[opp_d_(stencilIdx)]) = (1.0-_OMEGA_) * tempDstrb[stencilIdx] + _OMEGA_ * eq+
            (1.0-0.5*_OMEGA_) * fdist;
      }
   } );
   return;
...
q.parallel_for(sycl::range<1>{size_t(countIdx)},sycl::id<1>{size_t(startIdx)},[=,stencil_d_=this->stencil_d_,weight_d_=this->weight_d_,opp_d_=this->opp_d_](sycl::id<1> fluidIdx)
...
#endif
   {
	...
	tempDstrb[stencilIdx] = dstrb[nFluid * opp_d_[stencilIdx] + fluidIdx];
         ...
         v[2] += tempDstrb[stencilIdx] * stencil_d_[_DIMS_*stencilIdx+2];
#else  
	tempDstrb[stencilIdx] = dstrb[nFluid * opp_[stencilIdx] + fluidIdx];
         rho += tempDstrb[stencilIdx];      
...
#endif
         dstrb[nFluid * stencilIdx + fluidIdx] =
...
#ifdef USE_SYCL
		tempLoc[stencilIdx] = adjacency_d_[nFluid * stencilIdx + fluidIdx];
         tempDstrb[stencilIdx] = dstrb[tempLoc[stencilIdx]];
         rho += tempDstrb[stencilIdx];
         v[0] += tempDstrb[stencilIdx] * stencil_d_[_DIMS_*stencilIdx+0];
         v[1] += tempDstrb[stencilIdx] * stencil_d_[_DIMS_*stencilIdx+1];
         v[2] += tempDstrb[stencilIdx] * stencil_d_[_DIMS_*stencilIdx+2];
#else
         tempLoc[stencilIdx] = geometry_.adjacency_[nFluid * stencilIdx + fluidIdx];
         tempDstrb[stencilIdx] = dstrb[tempLoc[stencilIdx]];
         rho += tempDstrb[stencilIdx];
         v[0] += tempDstrb[stencilIdx] * stencil_[_DIMS_*stencilIdx+0];
         v[1] += tempDstrb[stencilIdx] * stencil_[_DIMS_*stencilIdx+1];
